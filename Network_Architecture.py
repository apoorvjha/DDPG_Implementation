...
class Network(Module):
    def __init__(self, config, mode = 'actor'):
        ...
        if self.config['NetworkArchitecture']['type'] == 'CNN':
            ...
            self.flat = Flatten()
            if mode != 'actor':
                self.dense_1 = Linear(256 * 10 * 10 + self.config['n_actions'], 256)
            else:    
                self.dense_1 = Linear(256 * 10 * 10, 256)
    ...
    def forward(self, x, x_action = None):
        ...
        x = self.max_pool(x) # (32, 128, 128) -> (32, 64, 64)
        x = ReLU()(self.cnn_3(x)) # (32, 64, 64) -> (64, 64, 64)
        x = Dropout(p = self.config['NetworkArchitecture']['p_dropout'])(x)
        x = ReLU()(self.cnn_4(x)) # (64, 64, 64) -> (64, 64, 64)
        x = self.max_pool(x) # (64, 64, 64) -> (64, 32, 32)
        x = ReLU()(self.cnn_5(x)) # (64, 32, 32) -> (128, 30, 30)
        x = Dropout(p = self.config['NetworkArchitecture']['p_dropout'])(x)
        x = ReLU()(self.cnn_6(x)) # (128, 30, 30) -> (128, 28, 28)
        x = self.max_pool(x) # (128, 28, 28) -> (128, 14, 14)
        x = ReLU()(self.cnn_7(x)) # (128, 14, 14) -> (256, 12, 12)
        x = Dropout(p = self.config['NetworkArchitecture']['p_dropout'])(x)
        x = ReLU()(self.cnn_8(x)) # (256, 12, 12) -> (256, 10, 10)
        x = self.flat(x) # (256, 10, 10) -> (256 * 10 * 10)
        if x_action is not None:
            x = torch.cat((ReLU()(self.dense_1(x)), x_action), dim = 0) # (256 * 10 * 10 + 1) -> 256
        else:
            x = ReLU()(self.dense_1(x)) # (256 * 10 * 10) -> 256
        x = Dropout(p = self.config['NetworkArchitecture']['p_dropout'])(x)
